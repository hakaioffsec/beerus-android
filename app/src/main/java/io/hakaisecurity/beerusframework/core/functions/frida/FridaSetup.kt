    ...
    companion object {
        ...
        fun getFridaVersions(onNewVersion: (String) -> Unit, onLoadingComplete: () -> Unit){
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if (fridaPages != null) {
                    val fridaVersions = fridaPages.lines().filter { it.contains("/frida/frida/tree/") }
        ...
        fun startFridaModule(activity: Activity,fridaVersion: String, active: String){
            ...
            runSuCommand("ls /data/local/tmp/hiddenBin"){ result ->
                if(result.contains("No such file or directory")){
                    downloadFromHttp(activity, "https://github.com/frida/frida/releases/download/$fridaVersion/frida-server-$fridaVersion-android-$arch.xz", active)
                    saveFridaCurrentVersion(activity, fridaVersion)
        ...
        private fun unzipXZ(context: Context, file: String, active: String){
            ...
            inputFile.inputStream().use { fis ->
                XZInputStream(fis).use { xzis ->
                    File(context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), file.dropLast(3)).outputStream().use { fos ->
                        var bytesRead: Int
            ...
            runSuCommand("cd $downloadFolder && mv $outputFilename /data/local/tmp/hiddenBin"){result2->
                println(result2)
                saveFridaCurrentVersion(context, outputFilename.split("frida-server-")[1].split("-android")[0])
                runKillFrida(active)
            }
